...
#warning "LpCoreVanTx: LP-core not supported on this target, using stub implementation"
...
private:
    ...
    private:
        ...
        void InternalSendFrame(const uint8_t retryCount, const uint8_t data[], const uint8_t length);
    public:
        LpCoreVanTx(gpio_num_t rxPin, gpio_num_t txPin, LP_VAN_NETWORK_SPEED networkSpeed) { _rxPin = rxPin; _txPin = txPin; _networkSpeed = networkSpeed; };
        ~LpCoreVanTx(){ };
        void Start(){ };
        void SendNormalFrame(const uint16_t identifier, const uint8_t data[], const uint8_t length, const bool requireAck){ };
        void SendReplyRequestFrame(const uint16_t identifier){ };
