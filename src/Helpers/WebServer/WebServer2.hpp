...
WebServer(
    ...
    esp_err_t startWebServer() {
        ...
        {
            RegisterEndpoints();
            //RegisterHandler("/", HTTP_GET, &WebServer::hello_get_handler);
            /*
            httpd_uri_t uri_get = {
                .uri      = "/",
                .method   = HTTP_GET,
...
/*
const char* WifiWebServer::TAG = "WifiWebServer";

// Global instance of the WifiWebServer class
WifiWebServer wifiWebServer;

extern "C" void app_main() {
    // Initialize Wi-Fi and Web Server
    wifiWebServer.init("yourSSID", "yourPASSWORD");

    // In a real application, you can wait for the connection to complete before starting the web server
    // Wait for IP address, then start the server
    esp_event_loop_create_default();  // If not already created
    esp_event_handler_register(IP_EVENT, IP_EVENT_STA_GOT_IP, [](void* handler_args, esp_event_base_t base, int32_t id, void* event_data) {
        wifiWebServer.startWebServer();
    }, NULL);
}
*/
