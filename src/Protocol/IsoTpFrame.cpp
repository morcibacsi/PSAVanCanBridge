...
uint8_t IsoTpFrame::send_sf()
{
    ...
    BusMessage msg{};
    msg.id = _txMsg.tx_id;
    msg.dataLength = _txMsg.len + 1;
    memcpy(msg.data, TxBuf, _txMsg.len + 1);

    return _canInterface->SendMessage(msg); // Add PCI length
}
...
uint8_t IsoTpFrame::send_ff()
{
    ...
    return _canInterface->SendMessage(msg);       // First Frame has full length
}
...
uint8_t IsoTpFrame::Receive(unsigned long millis, uint16_t canId, uint8_t len, const uint8_t buf[])
{
    currentTime = millis;
