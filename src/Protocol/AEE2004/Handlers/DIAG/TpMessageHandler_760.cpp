...
void TpMessageHandler_760::ReceiveFinished(unsigned long currentTime)
{
    ...
    if (_rxMsg.Buffer[0] == 0x61 && _rxMsg.Buffer[1] == _radioType)
    {
        ...
        memcpy(_carState->VIN_FOR_HEADUNIT, _rxMsg.Buffer+2, VIN_LENGTH);

        if (_feedbackSignalCallback != nullptr)
        {
            _carState->SAVE_CONFIG = true;
            _feedbackSignalCallback(FeedbackSignal::VinReadComplete);
        }
    }
...
void TpMessageHandler_760::SetRadioType(uint8_t radioType)
{
    switch (radioType)
    {
        case 1:
        {
            _radioType = ADDRESS_VIN_RD4_RD43;
            break;
        }
        case 2:
        case 3:
        {
            _radioType = ADDRESS_VIN_RD45;
            break;
        }
        default:
        {
