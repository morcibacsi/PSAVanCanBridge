...
class MessageHandler_120 : public IMessageHandler<MessageHandler_120>
{
    ...
    public:
        ...
        {
            CMB2004_120Struct packet;
            std::memcpy(&packet, message.data, sizeof(packet));

            if (packet.Status.data.block_number == 1)
            {
                CAN120Block1 block1{};
                std::memcpy(&block1, message.data, sizeof(block1));

                carState->AlertHistory1.asUint64 = block1.asUint64;
            }
            else if (packet.Status.data.block_number == 2)
            {
                CAN120Block2 block2{};
                std::memcpy(&block2, message.data, sizeof(block2));

                carState->AlertHistory2.asUint64 = block2.asUint64;
            }
            else if (packet.Status.data.block_number == 3)
            {
                CAN120Block3 block3{};
                std::memcpy(&block3, message.data, sizeof(block3));

                carState->AlertHistory3.asUint64 = block3.asUint64;
            }
        }
};
#endif
