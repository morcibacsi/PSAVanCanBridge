...
union CAN_1D0_Byte1Struct {
    struct {
        uint8_t ac_mode                : 3; // bit 0-2
        uint8_t fault_present          : 1; // bit 3
        uint8_t fan_mode               : 2; // bit 4-5
        uint8_t rear_window_heating_on : 1; // bit 6
        uint8_t off                    : 1; // bit 7  If this flag is 1 and bit 1 (outside_air) also set then the following message appears: Cabin air recycling activated
    } data;
    uint8_t asByte;
};

union CAN_1D0_Byte3Struct {
    struct {
...
struct CAN_1D0_Struct {
    CAN_1D0_Byte1Struct Modes;
    uint8_t Faults;
    uint8_t Speed;
...
union CAN_1D0_Packet {
    CAN_1D0_Struct data;
    uint8_t CAN_1D0_Packet[sizeof(CAN_1D0_Struct)];
};
